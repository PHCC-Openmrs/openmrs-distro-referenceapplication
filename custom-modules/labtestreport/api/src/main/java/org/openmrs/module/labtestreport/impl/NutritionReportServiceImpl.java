...
public class NutritionReportServiceImpl extends BaseOpenmrsService implements NutritionReportService {
	...
	public List<NutritionSummaryRow> getSummaryReport(Date startDate, Date endDate, boolean underFive) {
		...
		for (Object[] r : dao.getSummaryRows(startDate, endDate, underFive)) {
			...
			row.setCategory((String) r[5]);
			row.setAge(toInteger(r[6]));
			row.setLocation((String) r[7]);
			row.setVisitDate((java.util.Date) r[8]);
			row.setVisitCount(toLong(r[9]));
			row.setCurrentMuac(toDouble(r[10]));
			row.setLastMuac(toDouble(r[11]));
			row.setDiagnosis((String) r[12]);
			row.setTypeOfSupplement((String) r[13]);
			row.setSupplementQuantity(toDouble(r[14]));
			row.setNationalId((String) r[15]);
			row.setPhoneNumber((String) r[16]);
			row.setProject((String) r[17]);
			row.setStatus((String) r[18]);
			rows.add(row);
		}
		return rows;
	}
