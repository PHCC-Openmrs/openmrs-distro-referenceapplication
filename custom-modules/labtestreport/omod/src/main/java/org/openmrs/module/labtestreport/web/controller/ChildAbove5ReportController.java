...
/**
 * Renders the Nutrition Report - Child Above 5: one row per beneficiary currently 5 or older,
 * with their most recent visit's data, Current MUAC and Last MUAC. Clicking a row goes to that
 * beneficiary's O3 chart.
 */
@Controller
@RequestMapping("/module/labtestreport/nutritionChildAbove5.form")
public class ChildAbove5ReportController {
	...
	public ModelAndView showReport(@RequestParam(value = "startDate", required = false) Date startDate,
		...
		List<NutritionSummaryRow> summaryRows = Context.getService(NutritionReportService.class)
		        .getSummaryReport(startDate, endDate, false);
		...
		return new ModelAndView("/module/labtestreport/nutritionChildAbove5Report", model);
	}
}
