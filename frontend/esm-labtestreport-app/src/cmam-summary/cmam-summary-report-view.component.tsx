...
interface CmamSummaryReportViewProps {
...
// Alert-status patients need a doctor to schedule a follow-up - that's done as an Appointment,
...
/**
 * Shared rendering for both CMAM Follow-up summary reports (under 5 and above 5 - see
 * CmamAgeGroup): identical layout and behavior, differing only in which age group's data they
 * fetch and their page copy. See cmam-summary.component.tsx and cmam-above5-summary.component.tsx
 * for the two thin wrappers that supply those differences.
 */
export default function CmamSummaryReportView({
  ageGroup,
  pageTitle,
  countColumnLabel,
  exportFilenameBase,
}: CmamSummaryReportViewProps) {
  ...
  const { rows, isLoading } = useCmamSummaryReport(ageGroup, appliedDates.startDate, appliedDates.endDate);
  ...
  const mainExportSheet = useMemo<ExportSheet>(
    () => ({
      name: pageTitle,
      headers: [t('dimension', 'Dimension'), t('category', 'Category'), countColumnLabel],
      rows: visibleRows.map((row) => [dimensionLabels[row.dimension] ?? row.dimension, row.category, row.total]),
    }),
    [t, visibleRows, pageTitle, countColumnLabel],
  );
  ...
  const exportExtraSheets = useMemo<Array<ExportSheet>>(
    () =>
      (Object.keys(rowsByDimension) as Array<CmamDimension>).map((dimension) => ({
        ...
        headers: [t('category', 'Category'), countColumnLabel],
        rows: rowsByDimension[dimension].map((row) => [row.category, row.total]),
      })),
    [t, rowsByDimension, countColumnLabel],
  );
  ...
  return (
    <div>
      ...
      <div className={pageStyles.pageBody}>
        <h2 className={pageStyles.pageHeading}>{pageTitle}</h2>
